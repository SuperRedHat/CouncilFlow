...
4. **主控感知路由（0.2.0+）** — 角色配成 `controller` 时跟随"当前正在驾驶的 AI"本地执行、不起 sidecar；换个主控打开同一项目，分工自动跟着走
...
长任务恢复用两个 `wait` 子命令：`council discussion wait <id>` / `council delegation wait <id>`（shell 超时 ≠ 任务失败，子进程还在跑，artifact 会落盘——用 wait 轮询到完成）。
...
> **project-manager 能力（v1.5.1）**：状态 MCP 已从"线性 build 任务 + 日志"扩展为支持**运营 / 管理 / 跨项目**——任务终态（`cancelled` / `superseded`）配受审计的 `close_task` / `reopen_task`、任务优先级、批量操作、结构化「当前焦点」、富日志（`kind` / `tags`）、双完成率、以及跨项目只读 `project_dir` + `get_portfolio`；1.4+ 增加元数据补丁（`edit_task`/`edit_tasks`）、只读一致性检查 `lint_state` 与安全自动修复 `reconcile`、任务重命名 `rename_task`（级联改写引用 + 全图校验不部分写）、全状态文件原子写与损坏容错。它与 CouncilFlow 的集成契约见 [docs/integration.md](docs/integration.md) 的 *project-manager Task-State Contract* 段；完整工具清单见 AutoSkills 的 [mcp/project-manager/README.md](https://github.com/SuperRedHat/AutoSkills/blob/main/mcp/project-manager/README.md)。
...
roles:
  # `controller` 哨兵 = 跟随当前主控本地执行，不起 sidecar（0.2.0 起的默认模板）。
  # 想把某个角色固定派给别的模型，换成具体模型名即可。
  planner: controller
  architect: controller
  implementer: controller
  tester: claude          # 例：测试固定让 Claude sidecar 做
  reviewer: codex         # 例：评审固定让 Codex sidecar 做
  fixer: controller
  advisor: controller
...
providers:
  default:
    total_timeout_seconds: 7200   # 与 wait 子命令的 2h 预算对齐
...
要点：
- 模型名支持家族变体（0.2.1+）：`claude-sonnet` / `claude-haiku` / `gemini-2.5-pro` / `gpt-4o` 等会路由到对应适配器并透传给 CLI 的 `--model`
- 角色还支持**动态路由**（按 `task.complexity` 等条件选模型 + fallback 链），写法见模板注释
- 配置 schema 严格校验：未知键、不支持的 `output_language`、解析不到适配器的 `default_models` 都会在加载期报错而不是静默吞掉
...
- **0.2.0 — controller 哨兵**：`roles.<role>: controller` 跟随当前主控本地执行（不起 sidecar），默认模板全角色 controller，开箱即"主控全干"，按需固定个别角色给其他模型
- **0.2.1 — 审计加固批次**（两轮多智能体审计 + 人工验证的修复集）：
  - Claude prompt 改走 **stdin**（消除 Windows `cmd /c` 注入面与 8191 字符命令行上限）
  - 超时终止改杀**整棵进程树**（不再留下继续烧 token 的孤儿 node/codex 进程）
